...
#         if split=='train':
#             x, y = x[0:10000], y[0:10000]  # 取前10000个数据
#         x = torch.tensor(resize_imgs(x.numpy(), 32))  # 将图像转换成32 * 32的图像
#         x = (x.float()/255.).unsqueeze(1).repeat(1,3,1,1)  # 将单通道转化为rgb 3通道
#         with open(path, 'wb') as f:   # wb:以二进制写方式打开
#             pickle.dump([x, y], f)   # 将Python数据对象转化为字节流
#     with open(path, 'rb') as f:     # rb以二进制读方式打开
#         x, y = pickle.load(f)   # 将二进制文件转换成Python对象
#         if channels == 1:
#             x = x[:,0:1,:,:]
#
#     if ntr is not None:
